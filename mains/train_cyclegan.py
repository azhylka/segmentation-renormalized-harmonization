...
base_dir = 'C:/Users/azhylka/Projects/segmentation-renormalized-harmonization/'
sys.path.append(base_dir)
from data.rms_loader import RMSloader
...
def visualize_step(writer, losses, step, stage):
    ...
    writer = tensorboard_vis(summarywriter=writer, step=step, board_name=f'{stage}/cycle_crops',
    ...
    if opt.joint_seg:
        ...
        writer = tensorboard_vis(writer, step, f'{stage}/seg_cycle_crops',
...
if __name__ == '__main__':
    ...
    train_dataset = RMSloader(osp.join(base_dir, 'datasets/train.h5'),  src_key='src', trg_key='trg', is_train=True)
    val_dataset = RMSloader(osp.join(base_dir, 'datasets/val.h5'),  src_key='src', trg_key='trg', is_train=True)
    ...
    data_loader_train = torch.utils.data.DataLoader(dataset=train_dataset,
                                                    ...
                                                    num_workers=4,
                                                    pin_memory=True)
    print('Validation set: %d' % (val_dataset.__len__()))
    ...
    for epoch in range(opt.epoch_count, opt.epoch_count + opt.niter + opt.niter_decay + 1):
        ...
        for iteration, data in enumerate(data_loader_train):
            ...
            print('Src -> Trg, Step %s [Epoch %s/%s Iteration %s/%s] (lr G/D %.5f %.5f):' % (step, epoch, opt.niter + opt.niter_decay,
                                                                                                iteration, len(data_loader_train),
                                                                                                model.optimizer_G.param_groups[0]['lr'],
                                                                                                model.optimizer_D.param_groups[0]['lr']), losses)

            if step % 100 == 0:
                visualize_step(writer, losses, step, 'train')
        ...
        with torch.no_grad():
            acc_losses = {}
            for iteration, data in enumerate(data_loader_val):
                model.set_input(data)
                losses = model.get_current_losses()
                for loss_key, loss_val in losses.items():
                    if loss_key not in acc_losses:
                        acc_losses[loss_key] = []
                    acc_losses[loss_key].append(loss_val)
            losses = {}
            for loss_key, loss_vals in acc_losses.items():
                losses[loss_key] = np.mean(loss_vals)
            visualize_step(writer, losses, step, "val")
